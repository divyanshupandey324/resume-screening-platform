        ...
        async with semaphore:
            ...
            try:
                ...
                if not resume_text or not resume_text.strip():
                    resume_text = f"Resume filename: {file.filename}"
                ...
                # Return a safe placeholder result so other resumes succeed
                return {
                    "name": f"Error: {file.filename}",
                    "email": "Screening Failed",
                    "score": 0,
                    "skills": [],
                    "experience_years": 0.0
                }
            finally:
                # Always delete the temp file from disk
                try:
                    if os.path.exists(file_path):
                        os.remove(file_path)
                except Exception:
